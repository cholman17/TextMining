...
u = api.get_user(2365950774)
print 'Username: '+u.screen_name

new_tweets = api.user_timeline(screen_name = "OscarTheGrouch",count=200)
alltweets.extend(new_tweets)
oldest = alltweets[-1].id - 1
while len(new_tweets) > 0:
		print "getting tweets before %s" % (oldest)
		#all subsiquent requests use the max_id param to prevent duplicates
		new_tweets = api.user_timeline(screen_name = "OscarTheGrouch",count=200,max_id=oldest)
...
oscargrouch_tweets = open('oscargrouch_tweets.txt', 'w')
for tweet in outtweets:
  oscargrouch_tweets.write("%s\n" % tweet)

'''with open('oscargrouch_tweets.csv', 'wb') as f:
	writer = csv.writer(f)
