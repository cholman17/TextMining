...
new_tweets = api.user_timeline(screen_name = "MeCookieMonster",count=200)
alltweets.extend(new_tweets)
oldest = alltweets[-1].id - 1
while len(new_tweets) > 0:
		print "getting tweets before %s" % (oldest)
		#all subsiquent requests use the max_id param to prevent duplicates
		new_tweets = api.user_timeline(screen_name = "MeCookieMonster",count=200, include_rts = False, max_id=oldest)
...
print "+++++++++++++++++++"
cookie_tweets = open('cookiemonster_tweets.txt', 'w')
for tweet in outtweets:
  cookie_tweets.write("%s\n" % tweet)
